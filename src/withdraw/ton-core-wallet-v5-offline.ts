...
/**
 * Builds a Wallet V5 BOC using only local key material and parameters. For a
 * deployed wallet, pass the current seqno obtained before going offline.
 */
export function buildTonCoreWalletV5Boc(
  ...
  const seqno = params.seqno ?? (params.includeStateInit ? 0 : undefined);
  if (seqno == null) {
    throw new Error("--seqno is required for offline Wallet V5 BOC preparation unless --include-state-init is set");
  }
  assertIntegerInRange("wallet-v5 seqno", seqno, 0, 0xffff_ffff);
  const validUntil = seqno === 0
    ? 0xffff_ffff
    : params.timeout ?? Math.floor(Date.now() / 1000) + 60;
  ...
  return {
    ...
    messageHash: messageCell.hash().toString("hex"),
